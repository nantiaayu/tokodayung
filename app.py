...
def asosiasi_training():
    if request.method == 'POST':
        if 'file' in request.files:
            ...
            nama_barang = request.form.getlist('nama_barang')
            min_support = float(request.form['nilai_support'])
            file_path = session.get('file_path')
            
            if file_path:
                # Train model Apriori
                model_filename, model_data = train_apriori_model(file_path, nama_barang, min_support)
                save_model_history(nama_barang, min_support, model_filename)
                
                # Dapatkan barang-barang terkait dari model
                related_items = {item: model_data['related_items'].get(item, []) for item in nama_barang}
                
                flash('Model berhasil dibuat!', 'success')
                return render_template('manager/asosiasi_training.html', 
                                       nama_barang_list=nama_barang, 
...
def train_apriori_model(file_path, nama_barang_list, min_support):
    ...
    # Simpan barang terkait untuk setiap barang dalam model data
    related_items = {}
    if 'consequents' in rules.columns:
        for barang in nama_barang_list:
            related_rules = rules[rules['antecedents'].apply(lambda x: barang in x)]
            related_items[barang] = related_rules['consequents'].apply(list).tolist()
    else:
        related_items = {barang: [] for barang in nama_barang_list}
...
def save_model_history(nama_barang, nilai_support, model_filename):
    cur = mysql.connection.cursor()
    nama_barang_str = ', '.join(nama_barang)  # Ubah daftar nama barang menjadi string yang dipisahkan dengan koma
    cur.execute("""
        INSERT INTO model_history (nama_barang, nilai_support, model_filename, created_at)
        VALUES (%s, %s, %s, %s)
    """, (nama_barang_str, nilai_support, model_filename, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    mysql.connection.commit()
    cur.close()
...
@app.route('/add_package_from_itemset', methods=['POST'])
@login_required
@role_required('manager')
def add_package_from_itemset():
    itemset = request.form.get('itemset')
    if itemset:
        name_package = 'Package based on ' + itemset  # You can adjust the naming logic as needed
        total_price = 0  # Adjust the pricing logic based on your requirements
        discount = 0  # Set the default discount or adjust based on your business rules
        ...
        try:
            cur.execute("""
                INSERT INTO package (name_package, total_price, discount, gambar_packages)
                VALUES (%s, %s, %s, %s)
            """, (name_package, total_price, discount, 'default_image.jpg'))  # Adjust default image as needed
            mysql.connection.commit()
            
            flash('Package added automatically based on itemset: {}'.format(itemset))
            return redirect(url_for('manager_packages'))
        except Exception as e:
            flash('Error adding package based on itemset: ' + str(e))
            mysql.connection.rollback()

    # Redirect back to where you came from if no itemset or error
    return redirect(url_for('asosiasi'))

@app.route('/history_model', methods=['GET', 'POST'])
def history_model():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM model_history ORDER BY created_at DESC")
    model_history = cur.fetchall()
    cur.close()

    return render_template('manager/history.html', model_history=model_history)
# Fungsi asosiasi
